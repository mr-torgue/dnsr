...
func ConvertRR(drr dns.RR, expire bool) (RR, bool) {
	...
	switch t := drr.(type) {
	case *dns.SOA:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "SOA", utils.ToLowerFQDN(t.Ns), ttl, expiry}, true
	case *dns.NS:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "NS", utils.ToLowerFQDN(t.Ns), ttl, expiry}, true
	case *dns.CNAME:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "CNAME", utils.ToLowerFQDN(t.Target), ttl, expiry}, true
	case *dns.A:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "A", t.A.String(), ttl, expiry}, true
	case *dns.AAAA:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "AAAA", t.AAAA.String(), ttl, expiry}, true
	case *dns.TXT:
		return RR{utils.ToLowerFQDN(t.Hdr.Name), "TXT", strings.Join(t.Txt, "\t"), ttl, expiry}, true
	default:
		fields := strings.Fields(drr.String())
		if len(fields) >= 4 {
			return RR{utils.ToLowerFQDN(fields[0]), fields[3], strings.Join(fields[4:], "\t"), ttl, expiry}, true
		}
	}
	return RR{}, false
}

// convertDNSRR converts our RR into a normal dns.RR structure
func ConvertDNSRR(rr RR) dns.RR {
	r, _ := dns.NewRR(rr.String())
	return r
}
