...
// Caches resource records that can be used when resolving queries.
// Note that caching full messages does not always work since you need a better granularity sometimes (f.e. glue records) 
//
...
//   3. Integrates the rootzone file 
...
// TODO: add support for AA and AD flags
...
type Cache struct {
	...
	// negative cache
	ncache  *cache.Cache[*RRCacheItem]
	ncap    int
	nttl    time.Duration

	// positive cache
	pcache  *cache.Cache[*RRCacheItem]
	pcap    int
	pttl    time.Duration

	rootcache map[uint64]*RRCacheItem
}
...
func WithDebugLogger() Option {
	return func(c *Cache) {
		c.logger = utils.InitLogger(true)
	}
}
